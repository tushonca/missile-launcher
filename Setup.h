void setup() {
  ...
  while (!SD.begin(1, 10)){};
  SD.remove("INIT.log"); SD.remove("DATA.csv"); delay(100);
  myFile = SD.open("INIT.log", FILE_WRITE); myFile.println("SD OK"); myFile.close();
  myFile = SD.open("DATA.csv", FILE_WRITE); myFile.println(" "); myFile.close();
  /* ---#microSD --*/
  ...
  myFile = SD.open("INIT.log", FILE_WRITE); myFile.println("BME OK");
  startAltit = pressureToAltitude(bme.readPressure());
  myFile.print("start H = ");
  myFile.println(startAltit);
  myFile.close();
  /* ---#BME280 ---*/
  ...
  sBus.begin(); delay(10);
  myFile = SD.open("INIT.log", FILE_WRITE);
